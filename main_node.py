...
def main_worker(args, config):
    ...
    if args.dataset == "STRINGdb":
        path = 'data/STRINGdb_weighted.pt'
    if args.dataset == "CPDB":
        path = 'data/CPDB_weighted.pt'

    if args.dataset == "Tele_min":
        path = 'data/Tele_min_weighted.pt'

    if args.dataset == "Tele_max":
        path = 'data/Tele_max_weighted.pt'
    ...
    for idx in range(epoch):
        ...
        n_embedding,n_cl, e_pr, w_pr, c1, c2, c3, c4 = net(e, u, x.to(device))
        ...
        with torch.no_grad():
            ...
            if n_auprc > best_metrics['test_auprc']:
                ...
                #     plt.legend()
                #     plt.savefig("t_SNE_w_stringdb_HipGNN.pdf",  bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss', default='0.18') # The number represents the value of alpha, "auto" is Weight learner and "average" is Average weight.
    parser.add_argument('--n_train', default=0.8)
    parser.add_argument('--e_train', default=0.5)  #edge training ratio is fixed as 0.8
