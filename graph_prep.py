...
parser.add_argument('--dataset', default='tele_min', help='Name of the dataset (default: STRINGdb)')
...
elif args.dataset == "tele_min":
    save_path = 'data/graph_tele_min.bin'
elif args.dataset == "tele_max":
    save_path = 'data/graph_tele_max.bin'
...
# weights1=np.ones_like(weights)

N = new_graph.number_of_nodes()
adj_weighted = coo_matrix((weights, (src.numpy(), dst.numpy())), shape=(N, N))


# adj_weighted = coo_matrix((weights1, (src.numpy(), dst.numpy())), shape=(N, N))
...
if args.dataset == "tele_min":
    torch.save(data_to_save, 'data/Tele_min_weighted.pt')
elif args.dataset == "tele_max":
    torch.save(data_to_save, 'data/Tele_max_weighted.pt')